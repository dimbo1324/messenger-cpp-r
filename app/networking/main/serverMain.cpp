...
public:
    void start()
    {
        Networking::ServerSocket server(8080);
...
private:
    std::string processRequest(Networking::SocketType clientSocket, const std::string &request)
    {
        ...
        else if (command == "LOGIN")
        {
            ...
            iss >> login >> password;
            if (usersByLogin_.count(login) && usersByLogin_[login]->password() == password)
            {
